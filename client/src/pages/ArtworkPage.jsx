...
export default function ArtworkPage() {
  ...
  useEffect(() => {
    if (!user) return;
    api.get('/watchlist')
      .then(res => setWatched(res.data.some(w => Number(w.artwork_id) === Number(id))))
      .catch(() => {});
  }, [id, user]);
    ...
    socket.on('bid:new', (data) => {
      if (data.artworkId === parseInt(id)) {
        ...
        setLiveFeed(prev => [{
          ...
          bidder: data.bidderName || `User #${data.bidderId}`,
          time: data.timestamp,
        }, ...prev.slice(0, 4)]);
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 400px', gap: 40, alignItems: 'start' }}>
          {/* Left: Images */}
          <div>
            <ImageSlider3D images={artwork.images} />
                ...
                onClick={async () => {
                  try {
                    if (watched) {
                      ...
                      await api.post('/watchlist', { artwork_id: Number(id) });
                      setWatched(true);
                    ...
                    <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {bids.map((b, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '6px 8px', background: i === 0 ? 'rgba(200,150,42,0.1)' : 'transparent', borderRadius: 4 }}>
                          <span style={{ color: i === 0 ? 'var(--accent-warm)' : 'var(--text-muted)' }}>
                            {b.bidder_username || `User #${b.bidder_id}`}
                          </span>
